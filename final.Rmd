...
The trend of the total reported cancer cases of both sexes has continuously increased over the years. \
...
toplot %>% 
  pivot_longer(cols = c(rate, fitted_rate), names_to = "type") %>% 
  filter(age %in% c(52,72,87)) %>% 
  ggplot(aes(x = year, y = value, color = factor(age))) +
  geom_point(aes(shape = type), size = 0.5) +
  facet_wrap(~sex, ncol = 2) +
  theme_classic() + 
  coord_cartesian(expand = FALSE) + 
  ...
  #xlab("Year") + ylab("Number of cases per 10^5 population") 
  labs(
    title = "Incidence rate by year, age and sex, fitted and observed values",
    x = "Year",
    y = "Number of cases per 10^5 population",
    color = "Age"  # Change the legend title
  )
```
