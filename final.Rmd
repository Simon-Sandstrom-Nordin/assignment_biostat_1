...
```{r agegroup, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r age plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9}
cases %>% 
...
toplot <- cases_pop_ys %>% 
  add_column(fitted = model$fitted.values) %>% 
...
```{r }
toplot <- cases_pop %>% 
  add_column(fitted = model_age$fitted.values) %>% 
