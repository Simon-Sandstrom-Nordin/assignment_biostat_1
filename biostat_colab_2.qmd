...
Thresholds in the third figure (probabilities .4 to .8) are not clinically relevant as the net benefit is very close to zero in this interval which could be due to chance and not have meaningful implications.
...
7
The produeced model does not have statistical significance and would need further calibration to be meaningful. In order to calibrate, one could change the intercept while leaving the factors untouched.
